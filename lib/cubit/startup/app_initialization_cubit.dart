...
class AppInitializationCubit extends Cubit<AppInitializationStatus> {
  ...
  final String userContentUrl;

  final Map<String, String> _githubHeaders = const {
    'Accept': 'application/vnd.github.v3+json',
  };

  AppInitializationCubit(String organization, String repo, this.baseAssetName)
    : baseGitHubUrl = 'https://api.github.com/repos/$organization/$repo',
      userContentUrl = 'https://raw.githubusercontent.com/$organization/$repo',
  ...
  Future<int> _fetchRemoteBuildNumber(String tag) async {
    final String url = '$userContentUrl/$tag/pubspec.yaml';
  ...
  String _determineTargetAssetName() {
    if (isAndroidPlatform()) return '$baseAssetName-android.apk';
    if (isWindowsPlatform()) return '$baseAssetName-windows-x64.zip';

    if (isLinuxPlatform()) {
      String baseName = '$baseAssetName-linux-x64';
      switch (getLinuxDistributionFamily()) {
        case LinuxFamily.arch:
          return '$baseName.pkg.tar.zst';
        case LinuxFamily.debian:
          return '$baseName.deb';
        case LinuxFamily.unknown:
